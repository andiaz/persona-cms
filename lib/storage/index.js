...
/**
 * Screen type definitions with labels, icons, and colors
 */
export const SCREEN_TYPES = {
  landing: { label: 'Landing Page', icon: '🏠', color: 'blue' },
  login: { label: 'Login/Auth', icon: '🔐', color: 'purple' },
  dashboard: { label: 'Dashboard', icon: '📊', color: 'green' },
  form: { label: 'Form', icon: '📝', color: 'amber' },
  table: { label: 'Table/List', icon: '📋', color: 'slate' },
  detail: { label: 'Detail View', icon: '📄', color: 'cyan' },
  wizard: { label: 'Wizard/Stepper', icon: '🧙', color: 'indigo' },
  settings: { label: 'Settings', icon: '⚙️', color: 'gray' },
  profile: { label: 'Profile', icon: '👤', color: 'pink' },
  pricing: { label: 'Pricing', icon: '💰', color: 'yellow' },
  contact: { label: 'Contact', icon: '✉️', color: 'teal' },
  error: { label: 'Error Page', icon: '⚠️', color: 'red' },
  search: { label: 'Search', icon: '🔍', color: 'orange' },
  checkout: { label: 'Checkout', icon: '🛒', color: 'emerald' },
  modal: { label: 'Modal/Dialog', icon: '💬', color: 'violet' },
  other: { label: 'Other', icon: '📱', color: 'slate' },
};

/**
 * Get all site maps from localStorage
 * @returns {Array} Array of site map objects
 */
export function getSitemaps() {
  if (!isBrowser) return [];
  try {
    const data = localStorage.getItem(SITEMAPS_KEY);
    return data ? JSON.parse(data) : [];
  } catch (error) {
    console.error('Error reading site maps from storage:', error);
    return [];
  }
}

/**
 * Save all site maps to localStorage
 * @param {Array} sitemaps - Array of site map objects
 */
export function saveSitemaps(sitemaps) {
  if (!isBrowser) return;
  try {
    localStorage.setItem(SITEMAPS_KEY, JSON.stringify(sitemaps));
  } catch (error) {
    console.error('Error saving site maps to storage:', error);
    throw error;
  }
}

/**
 * Get a single site map by ID
 * @param {number} id - Site map ID
 * @returns {Object|null} Site map object or null if not found
 */
export function getSitemapById(id) {
  const sitemaps = getSitemaps();
  return sitemaps.find((s) => s.id === id) || null;
}

/**
 * Add a new site map
 * @param {Object} sitemap - Site map object
 * @returns {Object} The saved site map with ID
 */
export function addSitemap(sitemap) {
  const sitemaps = getSitemaps();
  const newSitemap = {
    ...sitemap,
    id: sitemap.id || Date.now(),
    name: sitemap.name || 'Untitled Site Map',
    viewport: sitemap.viewport || { x: 0, y: 0, zoom: 1 },
    screens: sitemap.screens || [],
    createdAt: sitemap.createdAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
 ...
 * @param {Object} updatedSitemap - Site map object with id
 * @returns {Object|null} Updated site map or null if not found
 */
export function updateSitemap(updatedSitemap) {
  const sitemaps = getSitemaps();
  const index = sitemaps.findIndex((s) => s.id === updatedSitemap.id);
  if (index === -1) return null;
...
/**
 * Delete a site map by ID
 * @param {number} id - Site map ID
 * @returns {boolean} True if deleted, false if not found
 */
export function deleteSitemap(id) {
  const sitemaps = getSitemaps();
  const filtered = sitemaps.filter((s) => s.id !== id);
  if (filtered.length === sitemaps.length) return false;
  saveSitemaps(filtered);
  return true;
}

/**
 * Duplicate a site map by ID
 * @param {number} id - Site map ID to duplicate
 * @returns {Object|null} The duplicated site map or null if not found
 */
export function duplicateSitemap(id) {
  const sitemap = getSitemapById(id);
  if (!sitemap) return null;

  const now = Date.now();
  // Deep copy and regenerate IDs for screens while preserving parent references
  const idMapping = {};
  const copiedScreens = (sitemap.screens || []).map((screen) => {
    const newId = `screen-${now}-${Math.random().toString(36).substring(2, 11)}`;
    idMapping[screen.id] = newId;
    return {
  ...
  // Update parent references with new IDs
  copiedScreens.forEach((screen) => {
    if (screen.parentId && idMapping[screen.parentId]) {
      screen.parentId = idMapping[screen.parentId];
    }
  });

  const newSitemap = {
    ...JSON.parse(JSON.stringify(sitemap)),
    id: now,
...
/**
 * Create a new screen object for a site map
 * @param {string} name - Screen name
 * @param {string} screenType - Screen type (from SCREEN_TYPES)
 * @param {string|null} parentId - Parent screen ID (null for root)
 * @param {number} order - Display order among siblings
 * @returns {Object} Screen object
 */
export function createScreen(name, screenType = 'other', parentId = null, order = 0) {
  return {
...
export function importData(data, options = { merge: false }) {
  ...
  const result = { personas: 0, journeyMaps: 0, impactMaps: 0, boards: 0, sitemaps: 0 };
  ...
  if (Array.isArray(data.sitemaps)) {
    if (options.merge) {
      const existing = getSitemaps();
      const existingIds = new Set(existing.map((s) => s.id));
      const newSitemaps = data.sitemaps.filter((s) => !existingIds.has(s.id));
      saveSitemaps([...existing, ...newSitemaps]);
      result.sitemaps = newSitemaps.length;
    } else {
      saveSitemaps(data.sitemaps);
      result.sitemaps = data.sitemaps.length;
    }
  }
...
export function clearAllData() {
  ...
  localStorage.removeItem(SITEMAPS_KEY);
}
...
export function getStorageInfo() {
  if (!isBrowser) return { used: 0, personas: 0, journeyMaps: 0, impactMaps: 0, boards: 0, sitemaps: 0 };
  ...
  const sitemapsData = localStorage.getItem(SITEMAPS_KEY) || '';

  return {
    used: personasData.length + journeyMapsData.length + impactMapsData.length + boardsData.length + sitemapsData.length,
    personas: getPersonas().length,
    journeyMaps: getJourneyMaps().length,
    impactMaps: getImpactMaps().length,
    boards: getBoards().length,
    sitemaps: getSitemaps().length,
  };
}
